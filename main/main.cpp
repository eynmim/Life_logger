/*
 * MOSA_MIC_PROJ — Tier 2: High-Quality Voice Engine (ESP-ADF)
 * ================================================================
 * Dual INMP441 microphones on XIAO ESP32-S3
 * ESP-ADF audio pipeline + ESP-SR AFE_VC NSNet + VADNet
 *
 * Audio Pipeline (ESP-ADF):
 *   I2S Stream (16kHz) → ESP-SR AFE_VC [NSNet + VADNet]
 *     → Post-processing [EQ / Noise Gate / Formant / Coherence]
 *     → Raw Stream → Ring Buffer → Output (Dashboard / WAV / A/B)
 *
 * Architecture:
 *   Core 1: Audio pipeline (I2S capture → AFE → post-proc → ring buffer)
...
static void init_afe() {
    serial_print("  Initializing ESP-SR AFE (HIGH_PERF + NSNet)...\n");
    ...
    // AFE_TYPE_VC: 1MIC + NSNet (neural noise suppression)
    ...
    // NSNet neural noise suppression
    cfg->ns_init = true;
    cfg->afe_ns_mode = AFE_NS_MODE_NET;
    cfg->ns_model_name = esp_srmodel_filter(models, "nsnet", NULL);
    ESP_LOGI(TAG, "NS model: %s", cfg->ns_model_name ? cfg->ns_model_name : "NONE");
    ...
    cfg->vad_min_speech_ms = 200;
    cfg->vad_min_noise_ms = 800;
    ...
    serial_printf("  [PRE-CHECK]  NS:%s mode:%d  SE:%s  VAD:%s\n",
                  cfg->ns_init ? "ON" : "off", cfg->afe_ns_mode,
                  cfg->se_init ? "ON" : "off", cfg->vad_init ? "ON" : "off");

    cfg = afe_config_check(cfg);

    serial_printf("  [POST-CHECK] NS:%s mode:%d  SE:%s  VAD:%s\n",
                  cfg->ns_init ? "ON" : "off", cfg->afe_ns_mode,
                  cfg->se_init ? "ON" : "off", cfg->vad_init ? "ON" : "off");
    ...
    serial_printf("  AFE ready: feed=%d nch=%d fetch=%d\n",
    ...
    serial_print("  Pipeline started on Core 1\n");
}
    ...
    if (wavABMode) {
        ...
        if (n_proc > 0) {
            ...
            for (int i = 0; i < n_proc; i++) {
                stereo_buf[2 * i]     = raw_buf[i];
                stereo_buf[2 * i + 1] = proc_buf[i];
            }
...
static void handle_commands() {
    while (serial_available()) {
        ...
        switch (c) {
            case '1': mode = '1'; printCount = 0;
                if (viewMode == 'D') serial_print("\n  >> Mic1\n\n"); break;
            case '2': mode = '2'; printCount = 0;
                if (viewMode == 'D') serial_print("\n  >> Mic2\n\n"); break;
            case 'B': mode = 'B'; printCount = 0;
                if (viewMode == 'D') serial_print("\n  >> Both\n\n"); break;
            case 'C': viewMode = 'D'; calibrate(true); printCount = 0; break;
            case 'D': viewMode = 'D'; printCount = 0; wavActive = false;
                serial_print("\n  >> Dashboard\n\n"); break;
            case 'P': viewMode = 'P'; wavActive = false;
                serial_print("\n  >> Plotter\n"); break;
            case 'W':
                viewMode = 'W'; wavABMode = false;
                serial_printf("\n  >> WAV: %ds, %dHz, mono\n", WAV_DURATION_S, SAMPLE_RATE);
                serial_print("  >> Start recorder, send any key...\n\n");
                while (!serial_available()) vTaskDelay(pdMS_TO_TICKS(10));
                serial_read(); start_wav_stream(); break;
            case 'A':
                viewMode = 'W'; wavABMode = true;
                serial_printf("\n  >> A/B: %ds, %dHz, stereo\n", WAV_DURATION_S, SAMPLE_RATE);
                serial_print("  >> L=raw R=processed. Send any key...\n\n");
                while (!serial_available()) vTaskDelay(pdMS_TO_TICKS(10));
                serial_read(); start_wav_stream(); break;
            case 'V': {
            ...
            case 'E': {
                bool en = !post_get_eq_enabled(); post_set_eq_enabled(en);
                serial_printf("\n  >> EQ: %s\n\n", en ? "ON" : "OFF"); printCount = 0; break;
            }
            case 'M': {
                bool en = !post_get_mmse_enabled(); post_set_mmse_enabled(en);
                serial_printf("\n  >> Noise Gate: %s\n\n", en ? "ON" : "OFF"); printCount = 0; break;
            }
            case 'F': {
                bool en = !post_get_formant_enabled(); post_set_formant_enabled(en);
                serial_printf("\n  >> Formant: %s\n\n", en ? "ON" : "OFF"); printCount = 0; break;
            }
            case 'G': {
                bool en = !coh_get_enabled(); coh_set_enabled(en);
                serial_printf("\n  >> Coherence: %s\n\n", en ? "ON" : "OFF"); printCount = 0; break;
            }
            case 'H':
                serial_print("\n  MOSA Tier 2 -- ESP-ADF Voice Engine\n");
                serial_print("  -----------------------------------\n");
                serial_print("  VIEW:  D=Dashboard  P=Plotter  W=WAV  A=A/B\n");
                serial_print("  MIC:   1=Mic1  2=Mic2  B=Both\n");
                serial_print("  AUDIO: C=Calibrate  V=VAD toggle\n");
                serial_print("  POST:  E=EQ  M=NoiseGate  F=Formant  G=Coherence\n");
...
extern "C" void app_main(void) {
    ...
    serial_print("\n  MOSA Tier 2 -- ESP-ADF Voice Engine\n");
    serial_print("  Dual INMP441 + ESP-SR NSNet + VADNet\n");
    serial_print("  16kHz | Neural noise suppression\n");
    serial_print("  [D]ash [P]lot [W]av [A]/B [C]al [H]elp\n\n");
